...
describe 'certmgmt::cert', :type => :define do
  ...
  context 'no params set' do
    ...
    it 'should fail if no cert given' do
  ...
  context 'with default params set' do
    ...
    it 'should report error on bad ' do
      params.merge!({'ensure' => 'badvalue'})
      expect { should compile }.to raise_error(RSpec::Expectations::ExpectationNotMetError,
              /ensure must be one of 'present', or 'absent'/)
    end
  $cert,
      $ensure   = 'present',
        $key      = undef,
          $chain    = undef,
              $certpath = $::certmgmt::certpath,
                $keypath  = $::certmgmt::keypath,
                  $certfile = "${title}.pem",
                    $keyfile  = "${title}.key.pem",
                      $owner    = $::certmgmt::owner,
                        $group    = $::certmgmt::group,
                          $certmode = $::certmgmt::mode,
